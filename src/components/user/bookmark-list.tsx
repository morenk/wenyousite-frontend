...
export function BookmarkList({
  ...
  if (isLoading) {
    return <div aria-label="正在加载收藏" role="status" className="divide-y divide-border py-5">{[0, 1, 2].map((item) => <Skeleton key={item} className="h-20 w-full rounded-[var(--radius-card)]" />)}</div>;
  }
