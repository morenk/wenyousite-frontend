...
describe("全局播放生命周期", () => {
  ...
  test("初始减少动态效果不会创建待播放任务，存储不可用仍支持当前页面省流量", () => {
    ...
    vi.advanceTimersByTime(300); expect(change).toHaveBeenLastCalledWith(true);
    ...
    setCoverDataSaver(false); vi.advanceTimersByTime(300);
  ...
  test("移除当前项后重选；卸载等待中的最后一项不再回调", () => {
    ...
    vi.advanceTimersByTime(300); expect(b).toHaveBeenLastCalledWith(true);
    disposers.pop()?.(); expect(b).toHaveBeenLastCalledWith(false);
    vi.advanceTimersByTime(300); expect(a).toHaveBeenLastCalledWith(true);
    window.dispatchEvent(new Event("scroll"));
    disposers.pop()?.();
    vi.advanceTimersByTime(1000); expect(a).toHaveBeenCalledTimes(2);
  });
  test("首屏等待300ms，两个列表仍只选择一张；嵌套滚动立即停止并取消旧计时", () => {
    const controller = new CoverPlaybackController();
    const a = register(controller, 100), b = register(controller, 450);
    vi.advanceTimersByTime(299); expect(b).not.toHaveBeenCalled();
    ...
    window.dispatchEvent(new Event("scroll"));
    vi.advanceTimersByTime(299); expect(b).toHaveBeenCalledTimes(2);
  ...
  test("初始省流量/reduced-motion不会闪播，偏好支持实时和跨标签同步", () => {
    ...
    setCoverDataSaver(false); vi.advanceTimersByTime(300); expect(change).toHaveBeenLastCalledWith(true);
  ...
  test("路由切换取消待播放；新页面提交前不因滚动恢复", () => {
    ...
    window.dispatchEvent(new Event("scroll"));
    vi.advanceTimersByTime(1000); expect(change).not.toHaveBeenCalled();
    controller.resumeRoute(); vi.advanceTimersByTime(300); expect(change).toHaveBeenLastCalledWith(true);
  ...
  test("模态弹层打开时停止，关闭后重选；全部卸载不残留计时", async () => {
    ...
    vi.advanceTimersByTime(300); expect(change).toHaveBeenCalledTimes(2);
    modal.remove();
