...
if (
  ...
  || formatting.relativeTime.relativeWindowSeconds !== 259200
  || formatting.relativeTime.sameYearFallback !== "MM-dd"
  || formatting.relativeTime.crossYearFallback !== "yyyy-MM-dd"
  || formatting.relativeTime.futureBehavior !== "absolute"
  || formatting.relativeTime.timezone !== "user-local"
  || formatting.relativeTime.exposureFormat !== "yyyy-MM-dd"
  || formatting.relativeTime.exposurePrecision !== "date-only"
  || !formatting.relativeTime.exactValueExposure.includes("web-accessible-name")
  || formatting.exactTime.format !== "yyyy-MM-dd HH:mm"
  || formatting.exactTime.timezone !== "user-local"
  || !formatting.exactTime.preserveExistingSeconds
) failures.push("Foundation 必须保留原始时间戳，普通内容只显示日期，精确记录保留时分与既有秒精度");
...
if (JSON.stringify(foundationContract.experiences.elements?.metadata?.topicTag) !== JSON.stringify({
  tone: "brand",
  prefix: "#",
  ...
  failures.push("Foundation 缺少纯文字粉色可点击主题标签契约");
}
if (
...
for (const token of ["--element-topic-tag-min-height", "--element-topic-tag-foreground", "--element-topic-tag-focus-ring", "--element-topic-tag-font-weight"]) {
  if (!read("src/components/thread/topic-tag-link.tsx").includes(`var(${token})`)) {
    failures.push(`主题标签必须消费 ${token}`);
  }
}
for (const capsuleClaim of ["rounded-full", "bg-[var(--element-topic-tag-surface)]", "border-[var(--element-topic-tag-border)]", "hover:bg-[var(--element-topic-tag-hover-surface)]"]) {
  if (read("src/components/thread/topic-tag-link.tsx").includes(capsuleClaim)) {
    failures.push(`主题标签不得恢复胶囊态：${capsuleClaim}`);
  }
}
