...
const worktree = realpathSync(resolve(dirname(file), ".."));
const directory = join(worktree, ".dev-preview");
const stateFile = join(directory, "session.json");
...
async function webIdentity(descriptor) {
  ...
  if (response.status !== 200 || response.headers.get("x-wenyou-preview-run") !== descriptor.runId
...
function result(state, status) {
  ...
  return { version: 1, kind: "wenyou-web-preview", status, task: state.task, worktree, sessionId: d.sessionId, runId: d.runId,
    ...
    ports: { backend: d.backend.port, media: d.media.port, web: d.web.port }, snapshot: d.snapshot,
...
async function daemon(token) {
  let state = load();
  if (state.token !== token || state.worktree !== worktree) throw new Error("预览启动登记不匹配");
  ...
  try {
    ...
    proxy = await previewProxy(descriptor);
    ...
    for (let i = 0; i < 120 && !stopping; i++) {
      try { await webIdentity(descriptor); registerMembers(); state = { ...state, status: "ready", readyAt: new Date().toISOString() }; save(state); return; }
...
async function main() {
  ...
  const [command = "start", ...args] = invocation;
  if (command === "--daemon") { await daemon(args[0]); return; }
  const options = {};
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--json") continue;
    if (!["--descriptor", "--task"].includes(args[i]) || !args[i + 1] || args[i + 1].startsWith("--")) throw new Error("用法：dev:preview start|status|stop --task ID [--descriptor /absolute/consumer.json] [--json]");
  ...
  if (!locked) {
    ...
    const child = spawn("flock", ["--nonblock", "--close", "--conflict-exit-code", "75", lock, process.execPath, file, "--lock-held", ...invocation], { cwd: worktree, stdio: "inherit" });
  ...
  }
  {
    ...
    if (command === "status") {
      ...
      if (status === "unavailable") { try { await webIdentity(previous.consumer); status = "ready"; } catch { /* 不向不可用代理交付地址。 */ } }
    ...
    if (previous && (owned(previous.supervisor) || owned(previous.next) || previous.members?.some(owned))) {
      ...
      await webIdentity(consumer); console.log(JSON.stringify(result(previous, "ready"))); return;
    }
    ...
    const state = { worktree, task: options.task, descriptor: descriptorPath, consumer, protocolSha: PREVIEW_PROTOCOL_SHA,
      token: randomUUID(), status: "starting", startedSource: source(), startedAt: new Date().toISOString() };
    save(state);
    ...
    for (let i = 0; i < 180; i++) {
      await sleep(500);
      const current = load();
      if (current.status === "ready") { await webIdentity(consumer); console.log(JSON.stringify(result(current, "ready"))); return; }
