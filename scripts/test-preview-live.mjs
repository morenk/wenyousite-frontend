...
async function cli(command, task = "preview-selftest") {
  const child = spawn(process.execPath, ["scripts/dev-preview.mjs", command, "--task", task, "--descriptor", descriptorFile], { cwd: root, stdio: ["ignore", "pipe", "pipe"] });
...
try {
  ...
  const api = await fetch(`${d.web.origin}/api/v1/health`); assert.equal(api.status, 503);
  ...
  writeFileSync(join(dir, "live-test.json"), JSON.stringify({ kind: "synthetic-fast-refresh", rounds: 3, preservedInput: true, sameDocument: true, digests, wrongTaskRejected: true, portConflictRejected: true, runtimeDisconnectBlocked: true, abnormalExitRecovered: true, nextLeaderExitCleaned: true, cleaned: true }, null, 2));
