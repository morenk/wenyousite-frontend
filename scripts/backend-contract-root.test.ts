...
test("指定后端来源必须存在且逐字校验，不回退相邻 checkout", () => {
  ...
  mkdirSync(contracts);
  for (const name of ["openapi.json", "internal-reference-v1-fixtures.json", "editor-clipboard-v2-fixtures.json", "thread-cover-media-v1-fixtures.json", "markdown-block-boundary-v1-fixtures.json", "markdown-editor-list-v1-fixtures.json"]) {
