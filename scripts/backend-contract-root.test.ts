...
test("指定后端来源必须存在且逐字校验，不回退相邻 checkout", () => {
  ...
  mkdirSync(contracts);
  for (const name of ["openapi.json", "internal-reference-v1-fixtures.json", "editor-clipboard-v2-fixtures.json", "thread-cover-media-v1-fixtures.json", "media-display-v1-fixtures.json", "markdown-block-boundary-v1-fixtures.json", "markdown-inline-combinations-v1-fixtures.json", "markdown-inline-combinations-v1.schema.json"]) {
  ...
  const run = (source: string) => execFileSync(process.execPath, [resolve("scripts/check-generated-api.mjs")], {
    cwd: process.cwd(), env: { ...process.env, BACKEND_CONTRACT_REF: "", WENYOUSITE_BACKEND_ROOT: source }, encoding: "utf8", stdio: "pipe",
  });
