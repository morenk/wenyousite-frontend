...
> **2026-08 交互调整**：入口由全局导航栏移至 Milkdown 顶部格式工具栏（登录可见）；编辑器底部仅保留 Markdown 提示和字数统计，不重复放置入口。草稿内容以 01–05 槽位托盘内嵌在当前编辑器下方，不再使用全屏遮罩或右侧 Sheet，避免浮动回复框的层叠上下文让全局导航穿透草稿层并导致编辑会话丢失。
...
- [x] 草稿托盘内嵌于当前编辑器，不产生全屏遮罩或与全局导航竞争层级
- [x] 空槽位显示空态提示；无草稿时显示槽位占位
- [x] 保存草稿到草稿池；满 5 槽时禁用自动分配并提示选择已有槽位覆盖
- [x] 恢复草稿把完整 content（含内联骰子节点）回填当前编辑器（无回调时复制完整正文）
- [x] 托盘打开期间继续编辑时，保存按钮始终读取当前编辑器的最新正文
