...
- 网络或版本冲突导致自动保存失败时关闭开关并持续显示错误状态，直到用户重新开启；编码失效时暂停并取消待发送旧值，恢复编码后继续
...
正文手动保存、覆盖确认后及自动队列真正发送前都从当前编辑器同步编码。编码失败不发送父表单中的旧 Markdown，也不覆盖最后有效草稿；尚未发送的队列任务失效，已发请求的响应只更新其已保存版本。编辑器卸载后不再发送排队写入。恢复不支持的草稿或在当前输入编码失效时恢复草稿，均保留当前输入和远端原草稿。

## 完整媒体展示与续查

正文草稿恢复同时传递授权正文的 `mediaDisplays`；编辑器只更新图片展示 DOM，保存/复制继续使用来源 Markdown。图片处理超时保留同一媒体记录的续查能力；切换账号清理恢复状态，重选相同字节可继续查询，不因超时重新直传。
