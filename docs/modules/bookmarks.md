...
实现收藏主题帖和动态、共享私有收藏夹分类、我的收藏管理、他人主题帖/动态收藏公开查看（尊重 `showBookmarks` 隐私）。

**当前能力：**
- 主题帖与动态详情首次收藏时先选择收藏夹；已收藏时直接取消
- `/bookmarks` 我的收藏管理页（主题帖/动态分栏，共用默认/自建收藏夹筛选、移动、取消收藏和 cursor 分页）
...
- `/bookmarks` 两个内容分栏共用当前分类；分类条固定“全部”在首位，分别显示主题帖/动态数量
