...
- 通知跳转精确打开主题帖楼层/楼中楼及动态主评论/楼中楼：帖子目标不在首屏分页时进入独立稳定阅读态，动态评论沿用评论上下文定位
...
通知筛选分组由 [`foundation.lock.json`](../../foundation.lock.json) 固定的 Foundation v6.6.1 契约中 `experiences.notifications` 定义，Web 直接消费生成产物，不在业务组件复制名称和成员：
...
- 详情页读取 `post` 参数后通过 `GET /posts/:id` 查询目标上下文。主题、父楼和精确回复在每次挂载时都强制复核，期间只显示稳定骨架与“正在打开目标楼层”，不展示缓存正文；403/404 会关闭编辑器、清除主题/楼层/回复缓存并显示统一不可访问态，绝不回落到仍可回复的父楼页面。复核成功后目标卡片与分页列表分离；“查看完整讨论”以 replace 清除 `post`，主楼层保留所属子贴并在首帧进入列表开头。
...
- 未登录跳 /login
- 类型筛选、精确定位与刷新策略（本轮）
- 帖子通知在目标重新校验后显示独立稳定卡片，不注入无限列表或启动滚动校准；查看完整讨论后首帧进入普通列表开头
- 乐观已读缓存更新（失败时自动回滚）
