...
- 详情页读取 `post` 参数后通过 `GET /posts/:id` 查询目标上下文。主题、父楼和精确回复在每次挂载时都强制复核，期间只显示稳定骨架与“正在定位目标楼层”，不展示缓存正文；403/404 会关闭编辑器、清除主题/楼层/回复缓存并显示统一不可访问态，绝不回落到仍可回复的父楼页面。复核成功后挂载完整讨论，在不透明遮罩下按 cursor 加载到真实目标；目标稳定对齐后才揭开，详情响应不会注入分页列表。
