...
| 组件 | 路径 | 说明 |
|------|------|------|
| SearchResults | `src/components/search/search-results.tsx` | 四类结果 Tab、分类 loading/error/empty、短词提示与动态/楼层加载更多 |
...
- 四类结果通过 Tab 切换；默认动态，仅激活的分类发起请求，已加载结果由 TanStack Query 缓存
...
- [x] `/search` 输入关键词搜索动态标题/正文、用户名、主题帖标题与楼层内容
- [x] 结果分「动态」「主题帖」「楼层内容」「用户」四个 Tab，均可切换和跳转
- [x] 默认只请求动态，其余分类在首次激活 Tab 时才请求
- [x] 动态与楼层单字符不请求，至少 2 字符才执行正文搜索
