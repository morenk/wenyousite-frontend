...
| GET | `/search/threads?q=&cursor=&limit=20` | OptionalAuth | 主题帖 Tab 按需查询，返回完整首页卡片并按相关度游标分页 |
| GET | `/search/users?q=` | OptionalAuth | 用户 Tab 按需查询，最多 20 条 |
| GET | `/search/posts?q=&cursor=&limit=20&includeBody=true` | OptionalAuth | 楼层 Tab 按需查询，相关度游标分页 |
| GET | `/threads/:threadId/search/posts?q=&cursor=&limit=20&includeBody=true` | OptionalAuth | 搜索本帖全部子贴中的楼层与楼中楼 |
| GET | `/search?q=` | OptionalAuth | 旧客户端兼容聚合端点，新前端不调用 |

> 各 Tab 不会并发预取，默认只请求动态，首次点击其他分类后才请求对应端点。动态、主题帖与楼层都使用游标分页，动态与楼层关键词至少 2 个字符；楼层每页 20 条且每个主题帖最多 3 条。用户结果排除注销账号，动态、主题帖与楼层仅返回公开内容。主题帖结果与首页复用完整 `ThreadCard`，封面仍只使用 `coverImages[0]`。用户与楼层结果的独立卡片采用 Foundation 10px 圆角、8px 卡片间距；主题帖连续列表只保留行分隔线。

> 主题帖和楼层 Tab 属于用户显式搜索，因此仍展示已注销作者留下的公开历史内容；这些搜索结果不会写入首页 `threads` 发现缓存。

> Web 的全站和帖内内容搜索显式开启 `includeBody=true`，覆盖主贴、子贴正文与楼层。旧客户端省略时仍只返回楼层；响应 `kind=BODY` 显示“正文”并跳转到对应子贴，`FLOOR` 保持原有定位。

> 帖内搜索使用相同的短词限制、相关度排序和游标，但不套用“每个主题帖最多 3 条”；PRIVATE 帖仅成员可搜，未发布帖仅楼主可搜。搜索结果携带 `parentPostId`，楼中楼可直接进入独立讨论页。

## 4. 响应结构

聚合兼容端点与分类端点都以固定 OpenAPI 和 `src/api/types.ts` 的生成类型为事实源。
...
搜索结果按查看者隔离，离开页面后保留 30 分钟缓存；切换账号时不复用上一位查看者的占位结果。拉黑或解除后取消相关在途查询并清除旧结果，再按当前权限重取。关键词变化时结果组件本身不重挂载；新请求期间保留上一组已完成结果、显示列表顶部更新线，避免内容区先清空再跳动。
...
帖内搜索标题使用“搜索本帖内容”，不另设检索范围副标题；仍搜索正文、楼层与楼中楼，保留最少字符限制和结果类型。
...
- 分类 Tab 只显示图标与名称，不显示已加载数量或伪装成总量的计数
...
- 用户项 → `/users/{id}`；主题帖项 → `/threads/{id}`；楼层项通过共享 `getPostHref` 携带目标帖子 ID 和 `parentPostId`，主楼层进入所属子贴完整列表并遮罩定位真实卡片，楼中楼直达对应独立讨论页并在完整回复列表中定位
...
`/search` 公开，无需登录。用户结果不返回邮箱等敏感字段且排除已注销账号；所有搜索按可选登录身份过滤双向拉黑关系，全站帖子结果不包含私密帖。帖内搜索继承当前主题帖权限：公开帖允许匿名，PRIVATE 帖仅成员可用，未发布帖仅楼主可用，无权访问时后端统一返回 404。
...
正文搜索在数据库执行超过两秒时返回可重试错误，不以截断结果伪装成功；页面沿用错误态和重试入口。

搜索主题帖封面与首页共用 [可见封面同时播放](home.md#11-列表动图封面)，使用 coverMedia 静态首帧；缺失元数据不请求原动画。
