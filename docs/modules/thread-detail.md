...
子贴目录状态写入稳定 URL：非默认子贴使用 `?subthread={id}`，默认子贴移除该参数；切换目录使用 shallow history replace，不发起 RSC 路由请求，并保留左右游标。当前子贴的相邻目录以及用户聚焦、悬停或按下的目标会预取首屏楼层，缓存命中时切换不重复请求。主楼层默认从早到晚，时间顺序使用与动态评论相同的单击按钮切换；最新在前时写入 `order=NEWEST`，使用 history push 并在页面内导航时保留。复制当前子贴链接只保留内容坐标，不携带排序状态。`post` 精确楼层参数优先于 `subthread`，二者并存时清理 `subthread`；目标重新校验完成后进入独立聚焦阅读态，不挂载分页列表。聚焦态的“查看完整讨论”以 shallow history replace 清除 `post` 并保留目标所属子贴，首帧直接对齐楼层控制行。无效子贴参数回落默认目录并清理。目录菜单不设置搜索框，打开后直接聚焦当前子贴项，可用方向键浏览、滚动承载几十个子贴并复制当前子贴链接。正文中的主题、子贴、楼层和回复链接统一按[站内传送门](./internal-references.md)内联显示。
...
> **通知精确定位**：主楼层使用详情页 `?post=`，楼中楼通知直接进入 `/threads/{threadId}/posts/{parentPostId}/replies?post={replyId}`。请求期间显示固定高度骨架与“正在打开目标楼层”，主题、父楼和目标回复每次挂载都强制向服务端复核，复核前不渲染缓存正文或回复入口；任一目标返回 403/404 时关闭当前编辑器、清除相关正文缓存并显示统一不可访问态，不能回落到仍可回复的父楼页面。验证通过后只呈现目标卡片及简洁主题／子贴上下文，不把目标插入无限分页列表，也不启动滚动或尺寸校准。楼中楼仍使用独立讨论路由，精确回复同样独立呈现。
...
> **目标稳定阅读态**：通知、传送门、搜索结果和最新发言生成的 `?post=` 坐标共用独立聚焦阅读态。目标卡片不依赖当前分页是否包含它，后台缓存、分页结果或媒体尺寸晚到都不会改变目标在页面中的位置；重复进入或切换目标只替换聚焦卡片。点击“查看完整讨论”后清除 `post`：主楼层保留所属 `subthread` 并在浏览器绘制前对齐楼层控制行；楼中楼保留独立讨论路径并从父楼层开头阅读。普通列表继续复用已有 TanStack Query 缓存，不把目标详情伪装成分页结果。
...
> **最新发言定位**：排头卡工具带与冻结阅读条共用同一动作，并统一使用 Foundation `navigation.explore` 导航指南针图标。点击时按需调用 `postsFindLatestInThread`，只以创建时间判断最新，编辑旧内容不会顶帖。`parentPostId=null` 时复用主楼层 `?post=` 聚焦阅读态，否则直达父楼层的独立回复页；不遍历子贴或分页。接口成功后先处理未提交编辑器确认，再关闭搜索、清除可能遮蔽目标的主楼层作者筛选并导航；取消放弃编辑时留在原位。加载中两个入口同步禁用，已知 `_count.posts=0` 时禁用；删除竞态或空主题显示明确 toast。
...
| FloorTargetFocus / ReplyTargetFocus | `src/components/thread/post-target-focus.tsx` | `?post=` 重新校验后的独立目标阅读态、稳定骨架与“查看完整讨论”入口 |
...
> **讨论阅读筛选**：主题帖主楼层与独立楼中楼都使用轻量右对齐工具行；时间顺序通过同一 `action.sort` 按钮在“最早在前 / 最新在前”间直接切换，不使用下拉框或“阅读方式”面板。两处都提供“只看某人”：主楼层候选只取当前子贴实际发布过主楼层的角色作者，楼中楼候选只取当前主楼层下实际回复过的角色作者；普通参与人和其他子贴/楼层的角色成员不会混入。候选接口与列表首页并行读取，筛选不裁剪主楼层卡片内的 3 条回复预览。切换条件使用新的查询缓存与第一页游标；精确目标在独立阅读态展示，不参与作者筛选或列表注入。
...
- 消息、传送门、搜索与复制链接等入口共用独立目标阅读态：复核期间不泄露缓存正文，目标不注入分页列表且不受图片、分页或滚动校准影响
- 头部可打开帖内搜索，覆盖全部子贴与楼中楼并支持游标加载更多
- 搜索结果可打开所属子贴的稳定目标卡片；楼中楼直达独立讨论页，查看完整讨论后首帧从父楼层开头阅读
