...
> **邀请传送门契约**：合法的 `/join/{16 位 token}` 及 `https://wenyou.site` 同源绝对地址可在帖子、动态和评论中显示为站内传送门；解析阶段不请求邀请预览或私密帖元数据。发布邀请传送门等同于分享该邀请凭据，目标可见性仍由加入页和既有权限逻辑决定。
...
主题帖正文、楼层和独立回复遵循 `foundation.lock.json` 对应的 Web 平台规范；当前实现为主阅读正文 `17px / 约 32px`、最大 40 个全角字宽，嵌套回复 `16px / 1.85`。正文、粗体、斜体与富文本标题统一使用 Noto Sans SC，粗体使用真实 700 字重；LXGW WenKai 500 只用于详情内容标题和页面/区块结构标题。Milkdown 编辑区使用同一套字号、行距和强调映射。
...
- 骰子在正文中是不可编辑但可移动/删除的无图标原子节点，与普通文字基线对齐且内部不换行。未结算显示 `{notation} = ?` 并使用浅黄警告底，正式结果显示 `{notation} = {total}` 并使用柔粉品牌底；多骰的逐项点数和正负修正进入完整 `aria-label`，不把长数组塞进行内正文，也不只靠颜色区分状态。
