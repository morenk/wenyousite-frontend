...
子贴目录状态写入稳定 URL：非默认子贴使用 `?subthread={id}`，默认子贴移除该参数；切换目录使用 shallow history replace，不发起 RSC 路由请求，并保留左右游标。当前子贴的相邻目录以及用户聚焦、悬停或按下的目标会预取首屏楼层，缓存命中时切换不重复请求。主楼层默认从早到晚，时间顺序使用与动态评论相同的单击按钮切换；最新在前时写入 `order=NEWEST`，使用 history push 并在页面内导航时保留。复制当前子贴链接只保留内容坐标，不携带排序状态。`post` 精确楼层参数优先于 `subthread`，二者并存时清理 `subthread`；新鲜校验通过后进入完整讨论列表，并在不透明遮罩下按 cursor 顺序加载到真实目标卡片，稳定对齐后一次揭开。无效子贴参数回落默认目录并清理。目录菜单不设置搜索框，打开后直接聚焦当前子贴项，可用方向键浏览、滚动承载几十个子贴并复制当前子贴链接。正文中的主题、子贴、楼层和回复链接统一按[站内传送门](./internal-references.md)内联显示。
...
- OWNER/COLLABORATOR 可从头部“更多帖子信息与操作”打开导出配置；首屏选择 TXT、Markdown 或两者及普通图片，作者、北京时间、主楼层号、回复目标和来源链接收在“更多导出选项”中；ZIP 及正文文件按帖子标题命名，分类使用展示名称，楼中楼标记为回复，表情仅保留文字
...
> **分类显示契约**：`thread.category` 是可空的动态 slug，不再是封闭枚举。主题列表和管理表单通过 `GET /thread-categories` 显示管理员配置的名称，并使用 Foundation 统一中性呈现；分类 API 不包含颜色字段。未知 slug 显示原 slug，空值显示“未分类”，不能导致页面崩溃。详情排头不重复展示分类或招募状态。

> **通知精确定位**：主楼层使用详情页 `?post=`，楼中楼通知直接进入 `/threads/{threadId}/posts/{parentPostId}/replies?post={replyId}`。请求期间显示固定高度骨架与“正在定位目标楼层”，主题、父楼和目标回复每次挂载都强制向服务端复核，复核前不渲染缓存正文或回复入口；任一目标返回 403/404 时关闭当前编辑器、清除相关正文缓存并显示统一不可访问态，不能回落到仍可回复的父楼页面。验证通过后挂载完整讨论，在不透明遮罩下逐页查找真实目标；目标卡片进入 DOM 并稳定对齐后才一次揭开，绝不把详情响应伪装或注入为列表项。楼中楼继续使用独立讨论路由，但同样展示完整回复列表。
...
> **目标稳定阅读态**：通知、传送门、搜索结果和最新发言生成的 `?post=` 坐标都进入完整讨论。页面先强制复核主题、父楼与目标归属，再以不透明遮罩隐藏且 `inert` 化阅读区；遮罩独占 cursor 分页，普通无限滚动哨兵暂停，确保每页只请求一次。真实目标卡片进入 DOM 后持续按块开头对齐，布局稳定约 180ms 才一次揭开、播报定位完成并以 `preventScroll` 聚焦卡片；揭开后仍跟随迟到媒体和前方布局变化，直到用户首次滚轮、指针、触摸或键盘输入才释放。约 5 秒仍未完成时显示慢提示与返回入口；分页耗尽、网络失败或目标消失时保持遮罩并提供重试。目标切换、后退及重复点击会启动新的定位会话，URL 与既有 API 不变，也不把目标详情伪装成分页结果。
...
> **最新发言定位**：排头卡工具带与冻结阅读条共用同一动作，并统一使用 Foundation `navigation.explore` 导航指南针图标。点击时按需调用 `postsFindLatestInThread`，只以创建时间判断最新，编辑旧内容不会顶帖。`parentPostId=null` 时复用主楼层 `?post=` 完整列表定位，否则直达父楼层的独立回复页；最新发言接口本身不遍历子贴或分页，进入页面后再由定位遮罩沿所属列表 cursor 找到真实卡片。接口成功后先处理未提交编辑器确认，再关闭搜索、清除可能遮蔽目标的主楼层作者筛选并导航；取消放弃编辑时留在原位。加载中两个入口同步禁用，已知 `_count.posts=0` 时禁用；删除竞态或空主题显示明确 toast。

> **主题帖链接契约**：公开主题帖可通过 `/threads/{threadId}` 访问根页面，详情头部提供复制主题帖链接入口。私密帖不显示普通复制链接；生成或刷新邀请链接只放在管理台「帖子设置 → 私密访问」，详情头部不重复提供邀请与删除入口。

> **档案导出契约**：导出只允许已发布主题帖的 OWNER/COLLABORATOR。弹窗首屏选择 TXT、Markdown、两者及普通媒体，其他选项收在“更多导出选项”；服务端按选择生成以帖子标题命名的 `.md`/`.txt`、可选 `media/` 与 `export-notes.txt`，ZIP 外层也使用帖子标题命名。标题中的文件系统禁用字符由服务端清理，分类使用展示名称，时间固定为北京时间，楼中楼不伪造楼层号，表情只保留替代文字。Web 不自行拼接正文或打包，避免与移动端产生不同的历史档案格式。

> **主题标签导航**：详情标题下只列举 `#标签名` 纯文本链接，不使用胶囊、边框或填充，字号沿用原元信息行；每个标签链接到 `/tags/{tagId}`。标签页只列出关联该标签的公开已发布主题帖，私密帖不会因标签关系出现在公开列表中。

> **邀请重复访问契约**：`/join/[token]` 读取预览响应的 `alreadyJoined`。每次挂载都先复核 token，完成前不展示缓存概要；已是成员时显示加载过渡并立即 `replace` 到 `/threads/{threadId}`。加入接口保持幂等；若确认页停留期间 token 失效，POST 的 403/404 会立即切换到“邀请链接无效或已失效”，不会进入主题。普通 `/threads/{id}` 链接绝不触发邀请预览，非成员统一按不存在处理。
...
| DiscussionTargetMask | `src/components/thread/discussion-target-mask.tsx` | `?post=` 新鲜校验后的完整列表定位遮罩；串行 cursor 翻页、失败/慢提示、稳定揭开、读屏播报与焦点交接 |
...
| ReplyDiscussion | `src/components/thread/reply-discussion.tsx` | 独立楼中楼阅读主体：原楼层保留 `#n` 定位与标准操作菜单，作者可直接在本页编辑原楼层；下方连续展示回复列表与共享浮动输入坞，不重复显示楼中楼说明标题 |
...
| useLatestThreadPost | `src/api/hooks/use-latest-thread-post.ts` | 无缓存的按需定位 hook；返回主楼层/楼中楼稳定导航所需的最小上下文 |
...
> 楼层卡片固定展示 API 内联数据中的前 3 条楼中楼回复；每条预览右上角保留完整三点菜单，登录用户可在底部原位回复。发布时间与主楼层回复入口位于正文和预览之间，使用留白而非分割线组织层级。预览正文始终完整显示，不按文字长度裁切或绘制渐变；只要真实回复数大于零，即使只有 1 条，也始终显示安静的「展开楼中楼（N 条）」入口进入独立讨论页。没有楼中楼回复时不显示空状态文案。
...
帖子管理、帖子设置、子贴编辑、成员权限及导出设置弹窗的标题统一使用系统字体 body 600，包括管理中显示的帖子与子贴名称；正式阅读标题保留系统字体 display 500，创建与草稿续写页面保留原角色、字号、行高和字重，字体家族统一为系统字体。

## 6.1 帖主管理面板

管理视图直接展示字段、目录和成员筛选，不重复 Tab 标题或显示“正文画布”等说明层。子贴正文仅保留“正文”标签；保存快捷键在按钮 Tooltip 中提示。成员即时生效及保存/冲突状态继续显示；协作者可见性字段保留“仅楼主可修改可见性。”，不另设能力介绍卡。公开帖不显示私密访问卡，选为私密但未保存时禁用邀请并说明原因。旧邀请失效与删除影响在操作确认中说明。
...
- 「帖子设置」采用内容主栏与 18rem 发布侧栏。标题、分区、状态、标签和主帖正文通过 `PATCH /threads/:id/aggregate` 原子保存；可见性仅楼主可改。协作者仍能看到只读可见性及字段旁的楼主专属修改提示。
...
主题帖正文、楼层和独立回复遵循 `foundation.lock.json` 对应的 Web 平台规范；当前实现为主阅读正文 `17px / 约 32px`、最大 40 个全角字宽，嵌套回复 `16px / 1.85`。完整阅读态保留普通段落内的 Markdown 软换行，与 Flutter 展示一致；列表摘要另行折叠为空格。正文、粗体、斜体与富文本标题统一使用系统字体 body，粗体请求 700 字重；display 500 只用于详情内容标题和内容页面/区块结构标题。Milkdown 编辑区使用同一套字号、行距和强调映射。
...
**页面布局：** 主题帖排头卡为不带分类色块的两段式信息面板（`ThreadDetailHeader`：上层为标题、纯文本标签与低频工具，不显示分类、招募状态、作者或发布时间；下层为禁止换行的子贴目录和图标互动工具带，目录标题弹性截断，左右游标首尾循环）→ 排头卡离开视口后出现阅读书签条（不重复主题标题，只保留目录、最新发言、搜索、回顶）→ 当前子贴标题与正文（不重复显示定位说明）→ 楼层列表（不额外显示“讨论 n 楼”标题）→ 语义上位于列表底部、视觉上浮在视口底部的轻量发布入口。
...
> **楼中楼回复与独立阅读**：详情页固定完整预览前三条楼中楼，不在主题流继续展开整串；只要存在回复就提供「展开楼中楼（N 条）」进入独立阅读页。登录用户仍可在预览内直接回复任一可见回复。通知、搜索与精确链接可直接进入独立页。独立页顶部保留主题帖/子贴/原楼层上下文，原楼层作者可从同页操作菜单直接打开唯一编辑器并保存正文，无需返回子贴楼层页；回复按普通楼层密度连续排列，每条回复也有显式原位回复按钮。存储语义不变，创建仍传 `parentPostId=主楼层 id`、`replyToPostId=目标 id`；编辑、删除和无限分页复用原有 hooks。

> **讨论阅读筛选**：主题帖主楼层与独立楼中楼都使用轻量右对齐工具行；时间顺序通过同一 `action.sort` 按钮在“最早在前 / 最新在前”间直接切换，不使用下拉框或“阅读方式”面板。两处都提供“只看某人”：主楼层候选只取当前子贴实际发布过主楼层的角色作者，楼中楼候选只取当前主楼层下实际回复过的角色作者；普通参与人和其他子贴/楼层的角色成员不会混入。候选接口与列表首页并行读取，筛选不裁剪主楼层卡片内的 3 条回复预览。切换条件使用新的查询缓存与第一页游标；精确定位期间暂时忽略作者筛选，以完整排序列表逐页找到真实目标。
...
| 40403 | 最新发言在请求期间被删除，或主题暂无有效楼层/回复 | toast "当前主题还没有楼层或回复" |
| 403 / 404 | 已打开的主题、楼层或回复失效 | 强制关闭编辑器、清除正文缓存并显示不可访问态 |
...
- 详情标题下只展示主题标签，不展示分类、招募状态、作者或发布时间
- 详情标签保持原元信息字号，使用无胶囊的 `#标签名` 链接；无标签时不保留空信息行
- 排头卡内可打开子贴目录并切换，当前子贴在按钮和菜单内均明确标记
- 排头卡与冻结阅读条都可跳到跨全部子贴按创建时间最新的主楼层或楼中楼；编辑旧内容不改变目标，当前目标重复点击仍重新居中
- 冻结阅读条不重复主题标题，只保留当前子贴导航、最新发言、搜索与回顶；最新发言请求中两个入口同步禁用
...
- 任意子贴正文、主楼层与楼中楼回复均保留首部、中间和尾部的连续空段；历史原始空行按兼容规则恢复，长内容始终完整显示且不提供高度折叠
- 未登录用户可浏览公开帖，不能发帖
...
- 帖子设置的发布侧栏在可见性下方提供“主贴发言权限”，复用子贴策略选项与共享 Select，楼主和协作者均可修改；权限和标题、正文等由同一次 aggregate 保存，取消选择、未保存离开和失败冲突沿用现有表单行为。仅默认子贴楼层及楼中楼受影响，其他子贴不变；新建和未发布草稿不提供该入口。
- 保存只在权限变更时传 `defaultSubthreadPostingPolicy`；编辑版本取自表单基线，后台刷新不替换基线，成功或确认载入最新版本后才更新。保存响应中的发言能力仅写当前访问者详情缓存，其他身份详情失效后自行刷新。
...
- 消息、传送门、搜索与复制链接等入口共用完整列表定位：复核期间不泄露缓存正文，遮罩下逐页找到真实目标，稳定后一次揭开
- 头部可打开帖内搜索，覆盖全部子贴与楼中楼并支持游标加载更多
- 搜索结果可打开所属子贴并定位真实楼层；楼中楼直达独立讨论页并在完整回复列表中定位真实回复
