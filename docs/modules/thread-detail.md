...
> **目标开头定位（候选／待验收）**：通知、传送门、搜索和最新发言共用 `useDiscussionTargetReveal`。长短楼层均从含作者信息的块开头展示，保留共享间距并避让实际高度的吸顶阅读栏；末尾空间不足时停在自然滚动底部，不额外增加空白。前方图片撑高、分页插入、阅读栏迟到与窗口变化可重新校准；用户滚轮、触摸、指针按下或键盘操作后立即停止，重新选择目标或排序／作者范围时才重新激活。复现与验证见[Web 定位候选](../verification/discussion-target-start.md)。
