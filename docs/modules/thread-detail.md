...
- OWNER/COLLABORATOR 可从头部“更多帖子信息与操作”打开导出配置，选择 TXT、Markdown 或两者后下载 ZIP；默认包含作者、北京时间、主楼层号、回复目标和站内普通媒体，分类使用展示名称，楼中楼标记为回复，表情仅保留文字
...
> **档案导出契约**：导出只允许已发布主题帖的 OWNER/COLLABORATOR。弹窗选择 TXT、Markdown 或两者，并配置作者、时间、主楼层号、回复目标、站内来源链接和普通媒体是否保留；服务端按选择生成 `thread.md`、`thread.txt`、可选 `media/` 与 `export-notes.txt`。分类使用展示名称，时间固定为北京时间，楼中楼不伪造楼层号，表情只保留替代文字。Web 不自行拼接正文或打包，避免与移动端产生不同的历史档案格式。
