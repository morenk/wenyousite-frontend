...
列表默认只加载静态缩略图；动图在悬停或键盘聚焦时切换为完整资产。帖子、动态评论和私聊纯表情统一最大 128px，私聊纯表情不带气泡背景。静态表情展示时优先请求 128px 缩略图；动图使用最长边不超过 512px 的服务端规范化 WebP，不请求用户上传的来源原图。陌生消息请求中的图片和表情在用户点击前都不加载。
...
发送时只引用服务端已有资产 ID 或规范化后的资产 URL，不重新上传图片字节，所以私聊发送与帖子提交保持小请求和快速响应。帖子 Markdown 保存的是独立 `StickerAsset` URL，不是用户最初上传的原图；展示层仍强制使用统一 128px 上限。

## 完整媒体展示与续查

规范表情只消费服务端 URL/`display`，不推测文件名或再次转码。编辑器使用独立 NodeView 投影展示资源；模型、复制和保存继续保留来源 URL 与 asset ID。表情导入重试按账号会话、用途和原文件 SHA-256 隔离；切换账号清除待导入媒体 ID 并中止批量任务，迟到完成不得导入新账号。
