...
- 关注列表取消关注后移出该行。粉丝列表回关后显示互关；取消关注仅改变自己的关注方向，该粉丝仍在原位；移除粉丝仅解除对方关注我，保留我的关注，不发送通知且允许对方再次关注。
- 仅移除粉丝弹出共享 Dialog，标题包含实际用户名；说明不通知、可重新关注且没有撤销。默认聚焦取消，提交中禁止关闭及重复写入，失败保留原行；成功移行后焦点进入下一行或列表标题。
- 同一查看者与目标共享写入忙碌状态，即使切换页签也不能发起冲突操作。成功后取消旧读取、修补两向列表并失效用户资料和本人计数；旧会话完成不向新会话反馈。
- 网络中断或服务端结果不明只读核对两个完整列表。确认目标状态达到预期则正常收敛；无法读回时锁定关系写入，提供“刷新核实”，成功核对后才能继续，不自动重放写入。
- 可选 `viewerIsFollowing` 与 `viewerIsFollowedBy` 缺失时显示关系状态暂不可用，不猜测回关；只在兼容后端就绪后发布消费者。

契约来自 Backend `ea1ff7e2c6baeae6bf0316e87812ba37bc823d7a`，Foundation 保持锁定 v7.1.0，无额外 Token 或依赖升级。组件与 hook 测试覆盖方向、确认、失败、跨挂载防重、账号变化、旧响应和未知结果核对；浏览器验收只使用正式隔离 runner。
