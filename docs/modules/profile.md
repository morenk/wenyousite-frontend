...
| DELETE | `/users/me/followers/:id` | Auth/write | 移除指定粉丝，幂等且仅解除对方关注我 |
...
用户公开资料、最近动态、创建帖、收藏和参与帖使用 60 秒新鲜期，并在离开页面后保留 30 分钟，返回上一页面时优先复用缓存；关注／粉丝列表使用查看者隔离的 60 秒新鲜期与默认缓存回收策略。其中会随身份变化的 OptionalAuth 数据在 query key 中加入查看者 ID，认证恢复或账号切换后不会复用匿名/其他账号的权限结果。关注、收藏、资料修改等写操作仍主动失效对应查询。
...
### 资料与隐私编辑（/me）— PATCH /users/me

简介和隐私在同一页面内分别保存，复用 `profileSchema`，不包含 username：
...
帖子和收藏 Tab 内不再重复同名标题；本人的“我的收藏夹”和新建收藏夹按钮与收藏类型切换同行；前者进入当前类型的私有目录，新建成功直接进入新夹。隐私项使用“公开最近回复 / 公开参与的主题帖 / 公开收藏”，只有收藏项附“收藏夹名称与归类仅自己可见。”；无说明的控件不输出 aria-describedby。头像裁剪仅显示预览与缩放；背景裁剪保留同图分别裁剪、一起保存的规则。

- `/users/[id]` 展示资料卡（头像/用户名/Bio/关注粉丝数）；不展示入站时间
...
- `/users/[id]/bookmarks` 复用首页主题帖卡片只读展示允许访问的收藏；本人显示“我的收藏夹”与新建入口，新建成功进入对应类型的新夹；无权限时不发起收藏请求
...
- 关注/拉黑自己不显示按钮
- 最近回复列表渲染（正文/楼层/楼中楼三态标识 + 帖子链接 + preview），**仅展示最近 5 条**，为空/未公开有占位；点击通过共享 `getPostHref` 精确定位到对应楼层/楼中楼/正文；资料头部、创作概览、最近回复及其加载骨架统一使用 Foundation 10px 内容卡圆角，贴边封面由外框裁切；概览两张独立卡和独立回复卡使用 8px 间距，主题帖连续列表只保留分隔线
...
本人公开主页进入 `/users/[id]/following` 或 `/users/[id]/followers` 时显示“关注／粉丝”共享页签与计数，保留深链、返回入口和各自的滚动位置；他人列表继续浏览。采用无卡片外框或阴影的紧凑扁平列表，头像 36px，昵称单行省略，等级使用现有元信息样式，分隔线从昵称处起。行内不重复展示关系，也不另起操作行。

- 复用 [Foundation 控件角色与 Web 映射](../design-system.md)：关注／回关使用 `Button default` 主操作，已关注／互相关注使用 `secondary` 浅底状态，更多使用 `ghost` 图标；本次关系控件与菜单项的实际点击区域均至少 48px。色彩、圆角、弹层与阴影沿用现有 Token，不增加全局样式变体。
- 状态按钮与更多图标打开同一个右对齐锚定菜单。菜单顺序为私聊、取消关注、移除粉丝、拉黑、举报；私聊仅在现有 `/meta` 的 `capabilities.directMessages === true` 时显示，取消关注仅在已关注时显示，移除粉丝仅在对方关注我时显示（包括关注页的互关用户）。先关闭菜单，再进入对应操作或既有私聊／用户举报入口。
- 关注列表取消关注后移出该行。粉丝列表回关后显示互关；取消关注仅改变自己的方向，该粉丝仍在原位；移除粉丝仅解除对方关注我，保留我的关注，不发送通知且允许对方再次关注。
- 移除粉丝、拉黑均用共享 Dialog 确认，默认聚焦取消，最终提交统一使用 destructive；提交中禁止关闭和重复写入。拉黑不取消任何关注方向，按服务端可见性重新读取相关列表、资料和内容缓存。
- 菜单支持方向键、Home／End、Escape 和触发位置焦点恢复。菜单直接写入后若行仍在，焦点进入该行当前可用关系按钮；移行后进入下一行或列表标题。账号或目标失效时关闭原菜单与确认框，旧响应不修改当前界面。
- 同一查看者与目标共享关注、取消关注、移除、拉黑和取消拉黑的忙碌状态；跨页签或组件不能发起冲突操作。成功后取消旧读取，更新或重取列表并失效资料与本人计数。
- 网络中断或服务端结果不明不重放写入：关注类操作只读核对完整列表，拉黑类操作只信任目标资料的权威 `isBlocked`。确认目标态后正常收敛；投影相反或读回失败仍保留结果不明锁，仅提供“刷新核实”。缓存重置不会释放锁；用户显式只读核实完成后才能继续，既有主页按钮也有核实入口。
- 可选 `viewerIsFollowing` 与 `viewerIsFollowedBy` 缺失时显示“状态未知”，不猜测关注方向或开放关系菜单。

契约来自 Backend `ea1ff7e2c6baeae6bf0316e87812ba37bc823d7a`，本次菜单复用已发布的私聊、拉黑、举报与能力端点，不新增 API 字段。Foundation 交互依据 [已提交规范](https://github.com/morenk/wenyousite-foundation/blob/7263db135530326ac42b206eeffbf6f179513a91/docs/interaction.md#本人关注与粉丝管理)，当时运行依赖锁定 v7.1.0，现行版本以 `foundation.lock.json` 为准。组件、hook 与隔离浏览器回归覆盖菜单顺序、权限与能力、焦点、方向、确认、跨挂载防重、切账号、旧响应和未知结果核对；浏览器写入仅使用正式隔离 runner。
