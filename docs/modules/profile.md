...
契约来自 Backend `ea1ff7e2c6baeae6bf0316e87812ba37bc823d7a`，本次菜单复用已发布的私聊、拉黑、举报与能力端点，不新增 API 字段。Foundation 交互依据 [已提交规范](https://github.com/morenk/wenyousite-foundation/blob/7263db135530326ac42b206eeffbf6f179513a91/docs/interaction.md#本人关注与粉丝管理)，运行依赖保持锁定 v7.1.0。组件、hook 与隔离浏览器回归覆盖菜单顺序、权限与能力、焦点、方向、确认、跨挂载防重、切账号、旧响应和未知结果核对；浏览器写入仅使用正式隔离 runner。
