...
- 滚动（含嵌套容器）立即恢复静态；实际滚动目标全部触发 scrollend 后等待 60 ms 安静期重选（新的滚动会撤销），wheel/touchmove 还须满足最近输入后 120 ms 静止，避免离散输入逐次产生 scrollend 而反复下载。未收到结束事件时使用停稳 120 ms 兜底；较晚的结束事件不延后已有截止时间。随后从可见面积至少 50% 的动图中，按封面中心到有效滚动视口中心的欧氏距离选一张。普通 overflow 裁切只影响可见面积；平距保留此前选中项，否则按稳定展示顺序。
...
- 选中项优先请求 coverMedia.previewVariants 中的列表专用动画 WebP。仅在真正选中时记录封面框 CSS 宽高和 DPR，两维都满足目标像素时选面积最小档，否则选最大档；窗口 resize 暂停后重新选档，无关重渲染不更换稳定播放源。档位缺失、null 或无有效数据时，仅沿可信 animated=true 且首帧成功的既有路径回退原动画；已选预览加载失败则恢复首帧，不再串行下载原图。
- 动画加载期间显示首帧，成功后隐藏首帧，避免透明动画叠出第一帧残影。停止时卸载动画实例，迟到加载不会恢复旧动画。浏览器仍可能完成已发出的请求或保留解码缓存，不承诺底层网络一定被取消。
- 静态首帧与动画原样使用服务端稳定 URL，不追加时间戳或重新生成对象 URL；使用原生 HTTP 缓存，不另建 fetch/blob/Service Worker 缓存。缓存期限由资源响应头决定，浏览器可因空间或隐私策略驱逐缓存；复用传输不代表免除解码成本。
- 分页注册、首帧就绪及无关 ResizeObserver 通知不重置待播放截止时间；选中封面真实尺寸变化仍暂停重选。未选中动画不预取，也不隐藏预播放来消耗 GIF/WebP 的有限循环；冷加载仍需网络传输，原生 img 的 load 不等于观察到动画像素变化。
...
共享 contracts/thread-cover-media-v1-fixtures.json 校验 GIF、动画 WebP、单/双档预览、缺档原图兼容、可信历史静态图、未知媒体和旧服务降级。行为浏览器验收使用全 API/图片本地拦截，验证中心选择、滚动、跨页、省流量及迟到响应，不写真实业务数据。另以临时本地 HTTP 服务承接全部 API/图片并代理候选站静态页面，避免 Playwright route 禁用 HTTP 缓存；服务器请求/字节计数验证反复挂卸、切换后返回及同会话新页面复用稳定 URL，并以跨文档 no-store 重复传输作对照。该小型图片 fixture 只证明请求复用，不用于推断真实上传图片的压缩率、内存占用或解码性能。列表消费后端可选生成的动画 WebP 预览，原始 coverImages/coverMedia.url 关联及详情原图保持；不新增动画 WebP 上传支持，不回填历史数据，也不修改 CDN 或域名。
