...
- 标题、正文、数字与短标签统一使用 Foundation 系统字体 `system-ui, sans-serif`；保留 display/body/utility 角色和原字号、行高、字重，utility 启用等宽数字。品牌文字沿用系统字体，不加载自有字体或外部字体服务。
...
|------|------|
| 未登录 | 正常浏览列表，隐藏"创建"入口 |
| 已登录 | 宽屏左栏用display 角色的“发布”按钮统一提供“发布主题帖 / 发布动态”，个人快捷入口集中到右栏；右栏隐藏时个人入口回到左侧图标轨道 |
