...
**Markdown 输入与复制策略：** 工具栏、“更多”面板和与其一一对应的显式键盘命令是结构化能力入口；手动键入任何 Markdown 定界符都不会触发 input rule。外部 Markdown、HTML、Word、富文本和拖放无论是否只含白名单格式，都按系统可见文字静默插入，保存重开仍不解析；本地文件粘贴/拖放被忽略，图片继续只能经工具栏上传。本站 clipboard v2 阅读/编辑片段会在重新执行节点、属性和 URL 白名单后恢复格式，v1 继续兼容但没有块对齐；单独一个合法本站地址仍创建传送门。
