...
- 编辑器底部信息栏左侧提供私有表情入口，与正文滚动区分离；面板通过视口级 Portal 浮在编辑器外壳上方，不受外壳圆角裁切，表情较多时只滚动内部网格且不向页面传递滚动，并在当前光标插入最大 128px 的原子节点；普通图片只允许选择本地文件上传，隐藏外部图片 URL 输入
...
| 编辑器完整 Markdown | 同步 ProseMirror 事务桥 + 内联扩展节点编解码 | 单一 content 受控状态，正文与节点共同保存；发布不会读取防抖前旧值 |
...
| Table | ❌ | 表格不属于正文结构，手写与外部粘贴保留为字面文本，历史异常原文只读保护 |
| Latex | ❌ | 禁用 |
| AI | ❌ | 禁用 |
...
**Markdown 输入与复制策略：** 工具栏和“更多”面板是面向指针操作的结构化能力入口；编辑区已有的显式快捷键仍直接作用于正文，不提供键盘聚焦或激活工具栏按钮的路径。手动键入任何 Markdown 定界符都不会触发 input rule。外部 Markdown、HTML、Word、富文本和拖放无论是否只含白名单格式，都按系统可见文字静默插入，保存重开仍不解析；本地文件粘贴/拖放被忽略，图片继续只能经工具栏上传。本站 clipboard v2 阅读/编辑片段会在重新执行节点、属性和 URL 白名单后恢复格式，v1 继续兼容但没有块对齐；单独一个合法本站地址仍创建传送门。

阅读态同一正文内的原生选区和楼层/回复菜单“复制内容”都会写入 clipboard v2 结构片段，并附带不含 Markdown 定界符、对齐标记、目标 URL 或隐藏身份的可见纯文本。合法顶层普通段落和 H2/H3 的居中/右对齐可在站内恢复；传送门、单人提及、`@全体玩家` 与骰子表达式同样保留，骰子复制粘贴生成新 ID 且不继承结果。阅读端图片和表情只复制 `[图片]` / `[表情]`，编辑器内部复制才保留媒体节点。未知版本、跨正文选区、跨应用、Web↔Android 和跨设备均静默降为纯文本；完整规则及跨端黄金语料见 `markdown-content-protocol.md` 与 `contracts/editor-clipboard-v2-fixtures.json`。

**Markdown 编解码策略：** 历史正文、草稿和剪贴板的安全降级只发生在进入编辑器之前；编辑器内部以 ProseMirror 文档为事实源。每次正文事务由一个同步桥统一序列化，软换行直接写为普通 LF，自定义节点由 Markdown AST handler 写出，输出端只做空图片、换行和空段落的无损规范化并执行白名单断言，不再调用发布净化器或依赖 `markdownUpdated` 防抖回调。

**顶栏窄容器策略：** 所有编辑器共用单行左对齐工具栏，直接测量编辑器实际容器，不按主题详情、楼中楼或管理页分别猜断点，也不启用横向滚动。PC 一行容得下时平铺正文样式、粗体、斜体、删除线、行内代码、无序列表、有序列表、对齐、链接、图片、引用、分隔线、骰子和正文草稿，同时隐藏“更多”；标准内容栏保留行内代码直达，把链接、对齐、无序列表和有序列表收入“更多”。继续变窄时先收纳正文草稿，再降级为核心栏并收纳删除线与其余次级能力。一级对齐按钮随当前块在左、居中、右三个图标间切换；纯图标“更多”托盘不显示分类、分隔线或常驻文字，对齐以三枚相邻图标组成一个复合控件，其他图标保持同一连续排列，名称仅在指针悬浮时显示。顶栏、标题菜单和“更多”托盘的能力按钮均退出 Tab 顺序，只接受指针选择；弹层仍保留 Escape 关闭这一通用安全行为。对齐只对顶层非空 P/H2/H3 生效，独立普通图片块在 Markdown v5 下也可对齐，并由服务端 Markdown v5 元数据门控。“更多”以触发按钮为真实锚点，按菜单实际尺寸贴边、自动翻转并随滚动更新；Flutter 客户端保持固定核心栏与原生底部面板，不照搬 PC 的全量平铺。标题选择菜单同样按视口定位，只提供正文、二级标题和三级标题；其他标题层级作为字面文本处理。
...
继续编辑草稿仅保留页面主标题和表单，不叠加“草稿内容”卡片标题。
...
- PC 宽栏功能：服务端已启用 Markdown v5；一行容得下时直接展示正文样式、粗体、斜体、删除线、行内代码、无序列表、有序列表、对齐、链接、图片、引用、分隔线、骰子和正文草稿，不显示“更多”
- 收纳状态功能：标准内容栏保留行内代码直达，纯图标“更多”托盘承载链接、对齐、无序列表和有序列表；更窄时再承载正文草稿、删除线及其余次级能力，始终不横向滚动；托盘不分区、不显示分隔线或常驻文字，图标名称由 Tooltip 提供，对齐用相邻三图标直接选择左、中、右
- 手动输入 Markdown 定界符和所有外部富文本粘贴/拖放都显示可见字符，保存重开不创建结构；本地文件不能绕过图片工具栏
- 本站 clipboard v2 阅读选区、整篇菜单与编辑器复制可恢复白名单格式及合法顶层块对齐，v1 无对齐兼容、原子身份、骰子新 ID、媒体 label 与纯文本回退逐条通过共享黄金用例
...
主题帖保存草稿和发布前同步读取当前编辑器；编码失败时保留新输入并显示未同步反馈，禁止提交上次成功正文。恢复编码后可重新保存或发布。未知历史内容或服务端写能力不足时原文只读保留，修改标题也不会绕过正文保护。退出时，未修改的只读原文可正常返回；有编码失败的新编辑则保留现场。

主题/子贴标题100、线传Markdown正文10000均按Unicode码点校验；标签保持后端1–20字符及ASCII字母数字、中文、下划线和#白名单。
