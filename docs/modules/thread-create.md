...
| Feature | 状态 | 说明 |
|---------|------|------|
| TopBar | ✅ | 顶部固定工具栏：标题选择器、粗体/斜体/删除线、链接/图片、引用/分隔线、骰子与正文草稿（全部中文标签），按钮悬浮带中文 tooltip |
...
| ListItem | ✅ | 保留列表 Schema 与节点视图以兼容历史正文；禁止新建无序/任务列表，有序列表维持现状 |
...
**无序列表兼容策略：** 顶栏不展示无序列表入口，并移除 `- / + / * + 空格` 输入规则与 `Mod-Alt-8` 快捷键；事务层阻止产生新的 `bullet_list` 容器。粘贴 Markdown 或 HTML 无序/任务列表时，列表项按顺序拆成普通段落，并保留可兼容的行内格式。历史无序/任务列表仍可加载、显示、保存和编辑，不对已存 content 做迁移。

**顶栏窄容器策略：** 所有编辑器共用单行左对齐工具栏，不按主题详情、楼中楼或管理页分别设置断点。常见窄栏通过紧凑的按钮槽位与分隔线保持一行；容器继续收窄时仅顶栏横向滚动，编辑正文不横向移动。标题选择菜单使用视口浮层定位，避免被滚动容器裁切；键盘焦点进入屏外按钮时自动推进顶栏滚动位置。
...
- 编辑器功能：粗体/斜体/删除线/行内代码/链接/标题/有序列表/引用；历史无序列表与代码块可渲染和校验，但编辑器不提供对应入口
...
- 顶栏按钮悬浮提示（通过 JS `title` 属性注入中文本地化 tooltip）
