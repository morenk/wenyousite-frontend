...
- PC 宽栏功能：服务端已启用 Markdown v4；一行容得下时直接展示正文样式、粗体、斜体、删除线、行内代码、无序列表、有序列表、对齐、链接、图片、引用、分隔线、骰子和正文草稿，不显示“更多”
