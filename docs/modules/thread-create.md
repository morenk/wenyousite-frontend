...
| CodeMirror | ❌ | 不装载增强代码编辑器；代码块仍可由 Markdown 语法或粘贴创建、保留和基础编辑，但不提供按钮 |
| ListItem | ✅ | 列表 Schema 与节点视图保持启用；无序/有序列表在 PC 宽栏直接展示，收纳状态进入“更多”，任务列表通过语法或粘贴进入 |
| Placeholder | ✅ | "开始输入…"（可通过 props 自定义） |
| Table | ❌ | 不装载表格行列操作 UI；GFM 表格仍可由语法或粘贴创建、保留和基础编辑，但不提供按钮 |
| Latex | ❌ | 禁用 |
| AI | ❌ | 禁用 |
...
**Markdown 输入与复制策略：** 工具栏入口不是协议白名单。手写语法、快捷输入、Markdown/HTML 复制和历史正文中的安全 CommonMark/GFM 结构均按原结构解析、保存和再次编辑，不因没有一级按钮而拆成普通段落。无序/有序列表在 PC 宽栏直接展示、溢出时收入“更多”；任务列表、代码块和表格仅通过语法或粘贴创建。任意 HTML 仍不执行，图片继续受站内媒体规则限制。

**顶栏窄容器策略：** 所有编辑器共用单行左对齐工具栏，直接测量编辑器实际容器，不按主题详情、楼中楼或管理页分别猜断点，也不启用横向滚动。PC 一行容得下时平铺正文样式、粗体、斜体、删除线、行内代码、无序列表、有序列表、链接、图片、引用、分隔线、骰子和正文草稿，同时隐藏“更多”；真实溢出时将低频能力收入“更多”，继续变窄时先收纳正文草稿、再收纳删除线。“更多”使用视口浮层；Flutter 客户端保持固定核心栏与原生底部面板，不照搬 PC 的全量平铺。标题选择菜单同样按视口定位，只提供正文、二级标题和三级标题；其他标题层级仍可从语法、粘贴和历史正文进入。
...
- 收纳状态功能：实际溢出时显示“更多”，承载链接、行内代码、引用、无序列表、有序列表、分隔线和骰子；继续变窄时再承载正文草稿与删除线，始终不横向滚动
- 任务列表、代码块和表格可由语法或粘贴创建并继续编辑，但不提供按钮
...
- 编辑器不可见空段落不会在发布后显示为字面 `<br />`，代码块中的 `<br />` 示例不被误删
- 表格与代码块的宽内容只在自身区域横向滚动，不撑宽页面
- 保存草稿成功更新 Thread 元数据
