...
**Markdown 输入与复制策略：** 工具栏、“更多”面板和与其一一对应的显式键盘命令是结构化能力入口；手动键入任何 Markdown 定界符都不会触发 input rule。外部 Markdown、HTML、Word、富文本和拖放无论是否只含白名单格式，都按系统可见文字静默插入，保存重开仍不解析；本地文件粘贴/拖放被忽略，图片继续只能经工具栏上传。只有带 `data-wenyou-clipboard="1"` 的本站阅读/编辑片段会在重新执行节点、属性和 URL 白名单后恢复格式，单独一个合法本站地址仍创建传送门。

阅读态同一正文内的原生选区和楼层/回复菜单“复制内容”都会写入 clipboard v1 结构片段，并附带不含 Markdown 定界符、目标 URL 或隐藏身份的可见纯文本。传送门、单人提及、`@全体玩家` 与骰子表达式可在站内恢复；骰子复制粘贴生成新 ID 且不继承结果。阅读端图片和表情只复制 `[图片]` / `[表情]`，编辑器内部复制才保留媒体节点。未知版本、跨正文选区、跨应用、Web↔Android 和跨设备均静默降为纯文本；完整规则及跨端黄金语料见 `markdown-content-protocol.md` 与 `contracts/editor-clipboard-v1-fixtures.json`。
...
- 本站 clipboard v1 阅读选区、整篇菜单与编辑器复制可恢复白名单格式；原子身份、骰子新 ID、媒体 label 与纯文本回退逐条通过共享黄金用例
