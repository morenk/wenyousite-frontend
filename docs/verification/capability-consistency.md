...
| --- | --- | --- |
| W01 | 评论只接收单个 `mediaId`；已选图仍显示“图片”，选择器和异步确认期间缺少同步防重入；取消后较晚完成的压缩/上传可能继续提交并清理新状态 | 已选图显示“更换图片”，继续提示单图；选择器打开即锁定，cancel/change释放，每次清空文件输入允许同图重选。请求按 AbortController 归属，旧结果及旧 finally 不影响新请求；进入评论提交后同步禁止移除。新增组件竞态回归及隔离浏览器用例 |
...
- 手工复验：首次选图→更换→取消→非法文件→同图重选；上传期间移除并换表情；断网恢复；发送失败重试；切页/关闭后旧请求完成；每次只附一个媒体且失败正文保留。
