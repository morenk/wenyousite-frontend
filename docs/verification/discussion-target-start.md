...
> 历史记录：本文描述 2026-09-12 已部署版本的分页列表内滚动定位。2026-09-25 起，帖子 `?post=` 精确链接在新鲜归属校验后先以不透明遮罩隐藏完整列表，再沿 cursor 找到真实卡片并复用本文的几何定位能力；当前行为以 [`thread-detail.md`](../modules/thread-detail.md) 为准。动态评论也继续复用本文记录的几何定位能力。
