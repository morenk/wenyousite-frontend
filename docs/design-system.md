...
```text
community ≥1280  [左导航 208] [中央内容 672] [右侧信息 272]
...
type PageWidth = "narrow" | "feed" | "content" | "workspace";
type PanelTone = "plain" | "soft" | "accent" | "floating";
```

- 普通页面使用左侧全局导航；右侧只显示现有账户、钱包和快捷入口，不虚构热门数据。
- 1024px 隐藏右栏并将左栏收为图标轨道；登录、注册和创建等核心动作仍必须可达。
- 消息、主题帖创建/编辑和管理使用 `workspace`，不强塞右侧信息栏。
...
- Markdown 粗体使用 700 字重和轻微字距。Markdown 斜体在中文界面中改用 LXGW WenKai 正体强调，避免浏览器合成斜体挤压汉字笔画。
- 引用块不继承 Typography 默认斜体；代码、表格降一级字号，但不得低于约 15px。
