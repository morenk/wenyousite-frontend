...
- `WenyouTime` 默认为 `mode="content"`，统一列表与详情时间：72 小时内显示“刚刚 / N 分钟前 / N 小时前 / N 天前”，之后同年显示 `MM-dd HH:mm`、跨年显示 `yyyy-MM-dd HH:mm`，完整本地时间写入 `title`。账务、安全、审计、处置、预约及到期信息显式使用 `mode="exact"`，正文与 `title` 均显示本地 `yyyy-MM-dd HH:mm`，不订阅相对时间刷新；两种模式均保留 `<time dateTime>` 原始时间戳。普通内容仍按已锁定 Foundation v7.0.0 展示，去除绝对时分及日期读屏需在 Foundation 新版正式发布后同步升级。`WenyouCount` 从一万起使用“万/亿”紧凑值，并向辅助技术保留精确数字。
