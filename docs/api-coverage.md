...
- 后端总量：102 个路径、130 个操作
- 本轮明确搁置：举报 3 个操作、管理后台 5 个操作
- 用户端审计范围：122 个操作
- 前端直接调用：107 个操作；其余 15 个操作均由聚合响应、等价端点、兼容入口、原生移动端或部署健康检查覆盖，不构成缺失的 Web 功能

本轮已补齐认证续期、正确退出、公开主题帖详情、用户点赞状态、THREAD/USER 订阅、双端登录终端、黑名单、账号注销、邀请加入、公开加入和退出玩家身份。举报和管理后台按产品决定继续搁置。

全部 130 个操作的成功响应现已引用稳定 operationId 命名的具名 envelope；分页响应显式包含 cursor meta。每个操作都声明统一 `ApiErrorEnvelope` 兜底错误，业务错误码由 OpenAPI 的 `BusinessErrorCode` 生成，不再保留 Reports/Admin 匿名成功响应债务。
...
| `GET /api/v1/meta` | 客户端契约/能力协商入口，当前 PC Web 与后端同批部署 | 未来 Flutter 启动流程调用；Web 构建固定契约版本 |
