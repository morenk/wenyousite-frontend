...
- 后端总量：180 个路径、225 个操作
- 用户端范围外：举报 1 个操作、管理后台 53 个操作
- 用户端审计范围：171 个操作
- Web 源码直接引用：196 个操作（用户端 151 个，举报/站务 45 个）
- 未直接引用：用户端 20 个、举报/站务 9 个；这些操作由聚合响应、等价端点、兼容入口、受限凭据流程或原生移动端覆盖，不构成缺失的 Web 功能
...
| `DELETE /api/v1/users/me/followers/{id}` | 后端兼容新增，本轮仅同步契约 | 粉丝移除界面由独立关系管理任务接入 |
