...
- 后端总量：132 个路径、167 个操作
- 用户端范围外：举报 1 个操作、管理后台 25 个操作
- 用户端审计范围：141 个操作
- 前端直接调用：126 个操作；其余 15 个操作均由聚合响应、等价端点、兼容入口、原生移动端或部署健康检查覆盖，不构成缺失的 Web 功能

举报和管理后台不属于当前 PC Web 用户端范围。其余覆盖结论由固定契约与前端调用源码实时推导，不以某次审计日期或发布版本为准。

全部 167 个操作的成功响应现已引用稳定 operationId 命名的具名 envelope；分页响应显式包含 cursor meta。每个操作都声明统一 `ApiErrorEnvelope` 兜底错误，业务错误码由 OpenAPI 的 `BusinessErrorCode` 生成，不再保留 Reports/Admin 匿名成功响应债务。
