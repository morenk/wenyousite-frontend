...
- 后端总量：185 个路径、232 个操作
- 用户端范围外：举报 1 个操作、管理后台 58 个操作
- 用户端审计范围：173 个操作
- Web 源码直接引用：202 个操作（用户端 152 个，举报/站务 50 个）
- 未直接引用：用户端 21 个、举报/站务 9 个；这些操作由聚合响应、等价端点、兼容入口、受限凭据流程或原生移动端覆盖，不构成缺失的 Web 功能
...
| 操作 | 不单独调用的原因 | 当前覆盖方式 |
|------|------------------|--------------|
| `GET /api/v1/image-gallery` | 跨楼层图集当前仅由原生移动端接入 | Web 同步固定契约和类型，现有查看器仍消费内容图片列表 |
| `GET /api/v1/mobile-releases` | 公开移动端版本历史由 Flutter 关于页展示 | Web 消费独立管理列表并维护版本说明 |
| `GET /api/v1/mobile-releases/{platform}/{buildNumber}` | 安装后当前版本说明由 Flutter 展示 | Web 管理详情包含草稿、确认及公开快照 |
...
| `POST /api/v1/stickers/imports/moment-comment-image` | 当前 Web 不提供从动态评论收藏图片为表情的入口 | 原生移动端后续接入 |
| `POST /api/v1/stickers/imports/moment-image` | 当前 Web 不提供从动态收藏图片为表情的入口 | 原生移动端后续接入 |
