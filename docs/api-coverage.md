...
- 后端总量：160 个路径、200 个操作
- 用户端范围外：举报 1 个操作、管理后台 49 个操作
- 用户端审计范围：150 个操作
- 前端直接调用：175 个操作；其余 15 个操作均由聚合响应、等价端点、兼容入口、受限凭据流程或原生移动端覆盖，不构成缺失的 Web 功能
...
举报提交、决定/申诉和管理后台由独立 PC Web 路由消费；移动端当前不提供站务 UI，但普通 Bearer 举报/决定/申诉契约已保持平台无关。新增调用时继续同步模块文档、固定 OpenAPI、hooks、页面权限和测试。
