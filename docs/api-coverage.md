...
- 用户端审计范围：71 个路径、97 个操作
- 前端直接调用：84 个操作；其余 13 个操作均由聚合响应、等价端点或部署健康检查覆盖，不构成缺失的用户功能

本轮已补齐认证续期、正确退出、公开主题帖详情、用户点赞状态、THREAD/USER 订阅、设备会话、黑名单、账号注销、邀请加入、公开加入、退出玩家身份和子贴标签管理。举报和管理后台按产品决定继续搁置。
