...
开发反馈默认由 agent 在任务 Worktree 启动或复用 `pnpm dev:preview`，交付可打开的 SSH 本机浏览器地址；使用 Backend 已提交的隔离消费者描述，禁止默认连接线上 3000/3001。预览启动属于本地开发，不是正式部署，不需要先合并或运行 production build。

样式和排版连续反馈保留 Fast Refresh 与当前页面状态，只运行直接相关组件检查；不得每次保存执行全量门禁、构建或推送。agent 记录源码 SHA/摘要、会话编号、快照时间和视口；没有实际画面时明确标记待视觉验收。详见 [开发预览](docs/modules/dev-preview.md)。

实现过程中先跑受影响测试；反馈收敛后，代码任务交付前运行：
...
实时开发预览不改变公网服务；下面是反馈收敛后的交付链路：

1. 实现并运行相关测试。
2. 运行 `pnpm check`；认证/权限、核心旅程或高风险变更补充独立资源与隔离账号的 `pnpm check:full`。
